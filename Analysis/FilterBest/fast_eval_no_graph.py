...
#################################Only works if prioirty function does not need the graph so should modify the prompt of specification to not have the graph as input##############################

def priority(node, G, n, s):
    return sum([(n-i)*(i+1)*int(bit == "1") 
                    for bit, i in zip(list(node), range(len(node)))]) 
...
def greedy_independent_set(n, s, num_workers=4, checkpoint_file="hi.txt"):
...
if __name__ == "__main__":
    n = 16
    s = 1
    num_workers = 80  # Use all available CPUs
    start_time = time.time()
    independent_set = greedy_independent_set(n, s, num_workers=num_workers)
