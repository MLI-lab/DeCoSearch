...
class ProgramsDatabase:
            ...
            async with message.process():
                ...
                if island_id is None:
                    ...
                    for i in range(len(self._islands)):
                        await self.register_program(program, i, data["scores_per_test"], data.get("expected_version", None))
                else:
                    # Register the program to the specific island
                    await self.register_program(program, island_id, data["scores_per_test"], data.get("expected_version", None))
    ...
    async def register_program(self, program: code_manipulation.Function, island_id: int, scores_per_test: ScoresPerTest, expected_version: int = None):
        ...
        if len(island['clusters']) < cluster_check_threshold:
            if self.function_body_exists(island['clusters'], program.clean_body()):
    ...
    def _register_program_in_island(self, program: code_manipulation.Function, island_id: int, scores_per_test: ScoresPerTest):
        island = self._islands[island_id]
    ...
    def function_body_exists(self, clusters, cleaned_body: str) -> bool:
